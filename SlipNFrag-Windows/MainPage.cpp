...
namespace winrt::SlipNFrag_Windows::implementation
{
	...
	void MainPage::CreateDevice()
	{
		...
		if (FAILED(D3D12CreateDevice(adapter.get(), D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&d3dDevice))))
		{
			com_ptr<IDXGIAdapter> warpAdapter;
			check_hresult(dxgiFactory->EnumWarpAdapter(IID_PPV_ARGS(&warpAdapter)));
			check_hresult(D3D12CreateDevice(adapter.get(), D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&d3dDevice)));
		}

...
int Datagram_SendMessage (qsocket_t *sock, sizebuf_t *data)
{
    ...
    if (maxsize > 0 && data->data.size() > maxsize)
...
int Datagram_SendUnreliableMessage (qsocket_t *sock, sizebuf_t *data)
{
	...
	if (data->data.size() > maxsize)
		Sys_Error("Datagram_SendUnreliableMessage: message too big %u\n", data->data.size());
#endif

	packetLen = NET_HEADERSIZE + data->data.size();
...
//		continue;
		...
		sequence = BigLong(packetHeader.sequence);
		packetsReceived++;
		...
		{
			packetHeader.length = BigLong(NET_HEADERSIZE | NETFLAG_ACK);
			packetHeader.sequence = BigLong(sequence);
			sfunc.Write (sock->socket, (byte *)&packetHeader, NET_HEADERSIZE, &readaddr);
...
void NET_Stats_f (void)
{
	...
	{
		for (s = net_activeSockets; s; s = s->next)
			if (Q_strcasecmp(Cmd_Argv(1), s->address.c_str()) == 0)
				break;
		if (s == NULL)
			for (s = net_freeSockets; s; s = s->next)
				if (Q_strcasecmp(Cmd_Argv(1), s->address.c_str()) == 0)
...
static void Test_Poll(void*)
{
	...
	while (1)
	{
		len = dfunc.Read (testSocket, net_message.data, &clientaddr);
		if (len < sizeof(int))
			break;

		MSG_BeginReading ();
		control = BigLong(*((int *)net_message.data.data()));
		MSG_ReadLong();
		if (control == -1)
			break;
...
JustDoIt:
	...
	for (n = 0; n < max; n++)
	{
		...
		*((int *)net_message.data.data()) = BigLong(NETFLAG_CTL | 	(net_message.data.size() & NETFLAG_LENGTH_MASK));
		dfunc.Write (testSocket, net_message.data.data(), net_message.data.size(), &sendaddr);
	}
	SZ_Clear(&net_message);
	SchedulePollProcedure(&testPollProcedure, 0.1);
}
...
static void Test2_Poll(void*)
{
	...
	len = dfunc.Read (test2Socket, net_message.data, &clientaddr);
	if (len < sizeof(int))
		goto Reschedule;
	...
	*((int *)net_message.data.data()) = BigLong(NETFLAG_CTL | (net_message.data.size() & NETFLAG_LENGTH_MASK));
	dfunc.Write (test2Socket, net_message.data.data(), net_message.data.size(), &clientaddr);
	SZ_Clear(&net_message);
...
JustDoIt:
	...
	*((int *)net_message.data.data()) = BigLong(NETFLAG_CTL | (net_message.data.size() & NETFLAG_LENGTH_MASK));
	dfunc.Write (test2Socket, net_message.data.data(), net_message.data.size(), &sendaddr);
	SZ_Clear(&net_message);
	SchedulePollProcedure(&test2PollProcedure, 0.05);
}
...
static qsocket_t *_Datagram_CheckNewConnections (void)
{
	...
	len = dfunc.Read (acceptsock, net_message.data, &clientaddr);
	if (len < sizeof(int))
		return NULL;
	...
	if (command == CCREQ_SERVER_INFO)
	{
		...
		*((int *)net_message.data.data()) = BigLong(NETFLAG_CTL | (net_message.data.size() & NETFLAG_LENGTH_MASK));
		dfunc.Write (acceptsock, net_message.data.data(), net_message.data.size(), &clientaddr);
		SZ_Clear(&net_message);
		return NULL;
	}
	...
	if (command == CCREQ_PLAYER_INFO)
	{
		...
		playerNumber = MSG_ReadByte();
		activeNumber = -1;
		for (clientNumber = 0, client = svs.clients.data(); clientNumber < svs.maxclients; clientNumber++, client++)
		{
			if (client->active)
			{
				activeNumber++;
		...
		MSG_WriteString(&net_message, client->netconnection->address.c_str());
		*((int *)net_message.data.data()) = BigLong(NETFLAG_CTL | (net_message.data.size() & NETFLAG_LENGTH_MASK));
		dfunc.Write (acceptsock, net_message.data.data(), net_message.data.size(), &clientaddr);
		SZ_Clear(&net_message);
...
static void _Datagram_SearchForHosts (qboolean xmit)
{
	...
	if (xmit)
	{
		...
		MSG_WriteByte(&net_message, NET_PROTOCOL_VERSION);
		*((int *)net_message.data.data()) = BigLong(NETFLAG_CTL | (net_message.data.size() & NETFLAG_LENGTH_MASK));
		dfunc.Broadcast(dfunc.controlSock, net_message.data.data(), net_message.data.size());
		SZ_Clear(&net_message);
	}

	while ((ret = dfunc.Read (dfunc.controlSock, net_message.data, &readaddr)) > 0)
	{
		if (ret < sizeof(int))
			continue;
...
static qsocket_t *_Datagram_Connect (const char *host)
{
	...
	for (reps = 0; reps < 3; reps++)
	{
		...
		MSG_WriteByte(&net_message, NET_PROTOCOL_VERSION);
		*((int *)net_message.data.data()) = BigLong(NETFLAG_CTL | (net_message.data.size() & NETFLAG_LENGTH_MASK));
		dfunc.Write (newsock, net_message.data.data(), net_message.data.size(), &sendaddr);
		SZ_Clear(&net_message);
		do
		{
			ret = dfunc.Read (newsock, net_message.data, &readaddr);

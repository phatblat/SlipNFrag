...
void COM_CheckRegistered (void)
{
	...
	Cvar_Set ("cmdline", com_cmdline.c_str());
...
// reconstitute the command line for the cmdline externally visible cvar
	...
	// case we need to add these, so we don't need an overflow check
		...
		{
			largv.push_back(safeargvs[i]);
			com_argc++;
		}
	}

	largv.push_back(argvdummy);
	com_argv = largv.data();
...
char    *va(const char *format, ...)
{
    ...
    while (true)
    {
        ...
        auto needed = vsnprintf(string.data(), string.size(), format, argptr);
        va_end (argptr);
...
void COM_Path_f (void)
{
	...
	for (s=com_searchpaths ; s ; s=s->next)
	{
		if (s->pack)
		{
			Con_Printf ("%s (%i files)\n", s->pack->filename.c_str(), s->pack->numfiles);
		}
		else
			Con_Printf ("%s\n", s->filename.c_str());
	}
...
void COM_CopyFile (char *netpath, char *cachepath)
{
	...
	out = Sys_FileOpenWrite (cachepath, true);
...
//
	...
	// is the element a pak file?
		...
		// look through all the pak file elements
			...
			for (i=0 ; i<pak->numfiles ; i++)
				if (!strcmp (pak->files[i].name.c_str(), filename))
				{       // found it!
					Sys_Printf ("PackFile: %s : %s\n",pak->filename.c_str(), filename);
					if (handle)
					{
						*handle = pak->handle;
					...
					{       // open a new file on the pakfile
                        Sys_FileOpenRead (pak->filename.c_str(), file);
						if (*file >= 0)
	...
	// check a file in the directory tree
		...
		// see if the file needs to be updated in the cache
			...
			com_filesize = Sys_FileOpenRead ((char*)netpath.c_str(), &i);
			if (handle)
				*handle = i;
			else
			{
...
// crc the directory to check for modifications
	...
	for (i=0 ; i<header.dirlen ; i++)
		CRC_ProcessByte (&crc, ((byte *)info.data())[i]);
	if (crc != PAK0_CRC)
		com_modified = true;

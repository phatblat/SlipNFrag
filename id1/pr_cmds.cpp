...
void PF_normalize (void)
{
	...
	new_length = value1[0] * value1[0] + value1[1] * value1[1] + value1[2]*value1[2];
...
void PF_traceline (void)
{
	...
	if (trace.ent)
		...
		pr_global_struct->trace_ent = EDICT_TO_PROG((edict_t*)sv.edicts.data());
}
...
// return check if it might be visible	
	...
	{
		RETURN_EDICT((edict_t*)sv.edicts.data());
		return;
	}
...
void PF_precache_sound (void)
{
	...
	for (i=0 ; i<sv.sound_precache.size() ; i++)
	{
		if (!strcmp(sv.sound_precache[i].c_str(), s))
			return;
	}
    sv.sound_precache.emplace_back(s);
}

...
{
    va_list argptr;
    static std::vector<char> data(1024);
    int needed;
    
    while (true)
    {
        va_start (argptr, fmt);
        needed = vsnprintf(data.data(), data.size(), fmt, argptr);
        va_end (argptr);
        if (needed < data.size())
        {
            break;
        }
        data.resize(needed + 1);
    }

    auto fd = Sys_FileOpenAppend(file);
    if (fd >= 0)
    {
        Sys_FileWrite(fd, data.data(), needed);
        Sys_FileClose(fd);
    }
}
...
// log all messages to file
	if (con_debuglog)
		Con_DebugLog(va("%s/qconsole.log",com_gamedir.c_str()), "%s", msg.data());
...
void Con_DrawNotify (void)
{
	...
	for (i= con_current-NUM_CON_TIMES+1 ; i<=con_current ; i++)
	{
		...
		if (time > con_notifytime.value)
			continue;
		text = (char*)con_text.data() + (i % con_totallines)*con_linewidth;
...
// draw the text
	...
	for (i= con_current - rows + 1 ; i<=con_current ; i++, y+=8 )
	{
		...
		if (j<0)
			j = 0;
		text = (char*)con_text.data() + (j % con_totallines)*con_linewidth;
